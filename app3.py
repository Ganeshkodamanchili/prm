...
mydb=mysql.connector.connect(host='localhost',user='root',password='Mysqlproject#$$%*(*)',db='prm')
@app.route('/')
def index():
...
def forget():
    if request.method=='POST':
        ...
        if count==1:
            ...
            cursor.close()
            if status!='confirmed':
               flash('please comfirm your email first')
